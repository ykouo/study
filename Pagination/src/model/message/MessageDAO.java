...
public class MessageDAO {
	...
	/*//페이징을 위한 LIST 불러오기
	public ArrayList<MessageSet> selectPaging(String userid,int page){
		ArrayList<MessageSet> datas = new ArrayList<MessageSet>();
		conn = DBCP.connect();
		int startNum =(page-1)*5+1; // 여기서 보여주는 행의걔수를 설정 
		int endNum = page*5;
		String sql;
		try {
			...
			if((userid==null) || (userid.equals(""))){ 
				sql= "SELECT * FROM ("
						+"SELECT * FROM ("
						+"SELECT ROWNUM ROW_NUM,MESSAGE.* FROM MESSAGE"
						+") WHERE ROW_NUM>=?"
						+") WHERE ROW_NUM<=?";
			...
			// 로그인 했다면 userid값을 넘겨받아 해당 userid를 가진 유저가 쓴 글을 보여준다. 
				...
				sql= "SELECT * FROM ("
						+"SELECT * FROM ("
						+"SELECT ROWNUM ROW_NUM,MESSAGE.* FROM MESSAGE WHERE USERID=?"
						+") WHERE ROW_NUM>=?"
						+") WHERE ROW_NUM<=?";
...
*/	
	...
	public int getCnt(String userid) {
		int cnt=0;
		String sql; 
		conn = DBCP.connect();
		try {
			...
			// 로그인 했다면 userid값을 넘겨받아 해당 userid를 가진 유저가 쓴 글을 보여준다. 
				...
				sql = "SELECT COUNT(*) AS CNT FROM MESSAGE WHERE USERID = ?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, userid);				
				//System.out.println("여기는 [2]");
			}
			...
			e.printStackTrace();
			System.out.println("여기는 MessageDAO의 getCnt");
		}finally {
			DBCP.disconnect(conn, pstmt);
		}
		return cnt;
